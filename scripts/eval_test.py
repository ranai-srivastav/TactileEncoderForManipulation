...
def parse_args():
    ...
    p.add_argument(
        "--ablate-single-active",
        action="store_true",
        help="Evaluate one active modality at a time: V-only, T-only, FT-only, G-only, GF-only.",
    )
    p.add_argument(
        "--include-all-modalities-row",
        action="store_true",
        help="When using --ablate-single-active, also evaluate the full modality set first.",
    )
...
def _metrics_dict(loss: float, acc: float, prec: float, rec: float, f1: float) -> dict:
    return {
        "loss": loss,
        "accuracy": acc,
        "precision": prec,
...
def _markdown_table(rows: list[dict]) -> str:
    header = "| Setting | Active Modalities | Loss | Accuracy | Precision | Recall | F1 |"
    divider = "| --- | --- | ---: | ---: | ---: | ---: | ---: |"
    body = [
        f"| {row['setting']} | {' '.join(row['active_modalities'])} | "
        f"{row['loss']:.4f} | {row['accuracy']:.4f} | {row['precision']:.4f} | "
...
def main():
    ...
    if not args.ablate_single_active:
        return

    if args.which not in ("test", "val"):
        print("[WARN] --ablate-single-active expects a single target split; using test.")
    elif args.which != "test":
        print("[WARN] --ablate-single-active is intended for --which test; using the requested split as-is.")
    if args.split == "random" and args.seed is None:
        print(
            "[WARN] Random split is unseeded in the original training code. "
            "Without --seed, this ablation is not reproducible and may not match the training run split."
        )

    target_loader = test_loader if args.which in ("test", "both") else val_loader
    target_name = "test" if args.which in ("test", "both") else "val"
    all_modalities = ["V", "T", "FT", "G", "GF"]
    rows = []

    def run_setting(setting: str, active_modalities: list[str]) -> None:
        model.modalities = set(active_modalities)
        loss, acc, prec, rec, f1 = evaluate(model, target_loader, criterion, device)
        row = {
            "setting": setting,
            "active_modalities": active_modalities,
            **_metrics_dict(loss, acc, prec, rec, f1),
        }
        rows.append(row)
        print(
            f"[{setting}] {target_name} loss={loss:.4f} acc={acc*100:.2f}% "
            f"prec={prec:.3f} rec={rec:.3f} f1={f1:.3f}"
        )

    print()
    print("Single-active modality ablation")
    if args.include_all_modalities_row:
        run_setting("all-modalities", list(args.modalities))

    for modality in all_modalities:
        run_setting(f"{modality}-only", [modality])
