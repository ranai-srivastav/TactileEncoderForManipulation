...
from model import ForceTorqueOnlyLSTM
...
def run_case(device: str) -> None:
    ...
    frames_per_sec = 1
    sensor_rate = 3
    ft_dim = sensor_rate * 6
    gripper_dim = sensor_rate * 2
    height = width = 224

    model = ForceTorqueOnlyLSTM(ft_dim=ft_dim, hidden_dim=32, dropout=0.1).to(device)
    model.train()
    ...
    ft = torch.randn(batch_size, timesteps, ft_dim, device=device, requires_grad=True)
    ...
    logits = model(tactile, rgb, ft, gripper, gripper_force)
    ...
    grad_norm = ft.grad.norm().item()
    assert grad_norm > 0.0, grad_norm
    print(f"{device}: ok, logits_shape={tuple(logits.shape)}, ft_grad_norm={grad_norm:.6f}")
