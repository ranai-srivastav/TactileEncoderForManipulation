...
def print_dataset_stats(dataset, train_set, val_set, test_set) -> None:
    ...
    def _print_split(name, samples):
        ...
        print(f'    {"-"*38}')
        for key, display in [('grasp', 'Grasp'), ('pose', 'Pose'),
...
def parse_args():
    ...
    p.add_argument('--no_freeze_backbone', dest='freeze_backbone',
                   action='store_false')
    p.add_argument('--print_every', type=int, default=50,
                   help='Print train/val metrics every N iterations')
    p.add_argument('--overfit',     action='store_true')
    p.add_argument('--model_save_path', type=str,
                   default='trained_models/best_2d.pt')
...
def main():
    ...
    if args.overfit:
        ...
        print(f"Split ({args.split}): train={len(train_set)}, "
    ...
    print(f"\nConfig: lr={args.lr}, wd={args.weight_decay}, "
          f"batch={args.batch_size}, iters={args.n_iters}, "
          f"freeze={args.freeze_backbone}, L={args.L}")
    print(f"Printing every {args.print_every} iterations\n")
    ...
    while iteration < args.n_iters:
        ...
        for tactile, rgb, _, _, _, labels, _, lengths in train_loader:
            ...
            if iteration % args.print_every == 0:
                ...
                print(f"[iter {iteration:5d}/{args.n_iters}]  "
                ...
                if val_f1 > best_val_f1:
                    ...
                    print(f"  -> New best val F1: {val_f1:.3f}  "
                          f"(saved to {args.model_save_path})")
